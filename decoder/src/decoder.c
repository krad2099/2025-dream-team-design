...
#pragma pack(pop) // Resume padding
...
/* 
 * For the updated simple_crypto API, we need to update our crypto example.
 * The new encrypt_sym function uses AES-GCM, which produces an output that includes:
 *   - A 12-byte IV, followed by the ciphertext, then a 16-byte authentication tag.
 * We also use SHA-256 instead of MD5, so the hash output is 32 bytes.
 */
...
void crypto_example(void) {
    // Use a 16-byte plaintext. "Crypto Example!" is 15 characters plus the null terminator.
    uint8_t plaintext[PLAINTEXT_LEN] = "Crypto Example!";  
    // The ciphertext buffer must be large enough for IV, ciphertext, and tag.
    ...
    // Zero out the key (in a real design, use a secure key)
    memset(key, 0, sizeof(key));

    // Encrypt the plaintext and print the resulting ciphertext (in hex)
    if (encrypt_sym(plaintext, PLAINTEXT_LEN, key, ciphertext) != 0) {
    ...
    // Compute a SHA-256 hash of the ciphertext
    if (hash(ciphertext, CIPHERTEXT_LEN, hash_out) != 0) {
    ...
    // Decrypt the ciphertext and print the resulting plaintext
    if (decrypt_sym(ciphertext, CIPHERTEXT_LEN, key, decrypted) != 0) {
...
int main(void) {
    ...
    while (1) {
        ...
        case LIST_MSG:
            ...
            #ifdef CRYPTO_EXAMPLE
                // Run the crypto example (for demonstration; remove in final design)
                crypto_example();
            #endif // CRYPTO_EXAMPLE

            // Print the boot flag (for demonstration; remove in final design)
            boot_flag();
            list_channels();
            break;
