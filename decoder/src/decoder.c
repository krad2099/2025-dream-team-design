...
void init_global_secret(void) {
    ...
    if (fread(global_secret, 1, sizeof(global_secret), f) != sizeof(global_secret)) {
        print_error("Failed to read global.secrets\n");
        fclose(f);
        while (1);
    }
    fclose(f);
...
#else
void init_global_secret(void) {
    // Read the secret from secure flash. In production, ensure that the
    // secret provisioned here exactly matches the secret generated by gen_secrets.py.
