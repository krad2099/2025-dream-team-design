...
// Removed secure_provision.h since we are no longer using secure provisioning.
...
#ifdef CRYPTO_EXAMPLE
// Load the global key directly from the global.secrets file.
// This file is generated by gen_secrets.py.
static uint8_t global_secret[16];
void init_global_secret(void) {
    FILE *fp = fopen("global.secrets", "rb");
    if (fp == NULL) {
        print_error("Failed to open global.secrets file.\n");
        exit(EXIT_FAILURE);
    }
    size_t bytes_read = fread(global_secret, 1, sizeof(global_secret), fp);
    if (bytes_read != sizeof(global_secret)) {
        print_error("Failed to read the complete key from global.secrets file.\n");
        fclose(fp);
        exit(EXIT_FAILURE);
    }
    fclose(fp);
